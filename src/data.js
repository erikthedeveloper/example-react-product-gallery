 ...
 */
const categories = ['Robots', 'Monsters', 'Robots v2', 'Kittens'].map(
  (name, i) => ({
    id: i + 1,
    name,
  })
  ...
  return products.concat(
    Array.from({length: PRODUCTS_PER_CATEGORY}).map(() => {
      ...
      const product = {
        ...
        images: {
          medium: `https://robohash.org/${productId}?size=175x175&set=set${
            category.id
          }`,
          large: `https://robohash.org/${productId}?size=390x390&set=set${
            category.id
          }`,
        },
